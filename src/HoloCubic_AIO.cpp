...
void setup()
{
...
#endif
#if APP_COUNTER_USE
    app_controller->app_install(&counter_app);
#endif
    ...
    // 检查 WiFi 列表中是否存在指定的 SSID
    for (int i = 0; i < numNetworks; i++)
    {
        if (WiFi.SSID(i) == app_controller->sys_cfg.ssid_0)
        {
            ssidFound = true;
            break;
        }
    ...
    {
        // WiFi 列表中不存在指定的 SSID
        // 直接跳过流程，执行其他操作，自启动APP
        app_controller->sys_cfg.auto_start_app = "WebServer";
    }
    else
    {
        if (app_controller->sys_cfg.auto_start_app == "Weather")
        {
            // 尝试连接 WiFi
            g_network.start_conn_wifi(app_controller->sys_cfg.ssid_0.c_str(), 
