...
void app_control_gui_init(void)
{
    ...
    //'lv_style_set_text_opa' 函数用于设置文本的透明度。
    // LVGL中的透明度级别用LV_OPA_XXX宏表示，其中'LV_OPA_COVER'表示完全覆盖，即完全不透明。
    lv_style_set_text_opa(&app_name_style, LV_OPA_COVER);
    // 设置字体的颜色
    lv_style_set_text_color(&app_name_style, lv_color_white());
    // 'lv_font_montserrat_24' 是Montserrat字体，24是指字体大小为24
    lv_style_set_text_font(&app_name_style, &lv_font_montserrat_24);

    // APP图标页
    // 通过调用 lv_obj_create(NULL) 来创建一个空的屏幕对象，并将其赋值给 app_scr 变量。
    // 这样，你可以使用 app_scr 对象来添加其他对象，并将其显示在屏幕上。
    app_scr = lv_obj_create(NULL);
    lv_obj_add_style(app_scr, &default_style, LV_STATE_DEFAULT);
    // 设置不显示滚动条
    // 函数设置 app_scr 对象的背景颜色不透明度，以达到不显示滚动条的效果
