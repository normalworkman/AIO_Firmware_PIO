...
{
    if (ACTIVE_TYPE::UNKNOWN != act_info->active && ACTIVE_TYPE::SHAKE != act_info->active)
    {   
    ...
    if (0 == app_exit_flag)
    {
        ...
        if (ACTIVE_TYPE::GO_FORWORD != act_info->active) // && UNKNOWN != act_info->active
        {
            app_control_display_scr(appList[cur_app_index]->app_image,
                                    appList[cur_app_index]->app_name,
                                    anim_type, false);
            // FreeRTOS的vTaskDelay函数,在当前任务中引入一个200us延迟,在这个延迟期间，CPU可以被用来执行其他的任务
