...
#define SETING_CSS ".input {display: block;margin-top: 10px;}" \
                   ".input span {width: 300px;float: left;height: 36px;line-height: 36px;font-size:90%;}" \
                   ".input input {height: 30px;width: 180px;border-radius: 8px;}"  \
                   ".input .radio {height: 16px;width: 16px;margin-right: 10px;margin-left: 10px;}"  \
                   ".input .select {height: 30px;width: 180px;border-radius: 8px;text-align:center;}"  \
                   ".btn {width: 120px;height: 35px;background-color: #8F3430;border: 0px;color: #ffffff;margin-top: 15px;margin-left: auto;margin-right: auto;border-radius: 8px;}" \
                   "input[type=\"range\"] {-webkit-appearance: none;width: 180px;height: 10px;background-color: #ccc;border-radius: 5px;}"\
                   "input[type=\"range\"]::-webkit-slider-thumb {-webkit-appearance: none;width: 20px;height: 20px;background-color: #8F3430;border-radius: 20%;cursor: pointer;}"\
                   "input[type=\"range\"]::-moz-range-thumb {width: 20px;height: 20px;background-color:  #8F3430;border-radius: 20%;cursor: pointer;}"

#define SYS_SETTING "<form method=\"GET\" action=\"saveSysConf\" style=\"display: flex; flex-direction: column;\">" \
                    "<label class=\"input\"><span>WiFi Name(2.4G)</span><input type=\"text\"name=\"ssid_0\"value=\"%s\"></label>" \
                    "<label class=\"input\"><span>WiFi Password</span><input type=\"text\"name=\"password_0\"value=\"%s\"></label>"  \
                    "<label class=\"input\"><span>Location(中文)</span><input type=\"text\"name=\"tianqi_addr\"value=\"%s\"></label>" \
                    "<label class=\"input\"><span>开机APP</span><select class=\"select\" name=\"auto_start_app\"><option value=\"0\" %s>天气</option><option value=\"1\" %s>相册</option><option value=\"2\" %s>媒体播放</option></select></label>" \
                    "<label class=\"input\"><span>天气主题</span><input class=\"radio\" type=\"radio\" value=\"0\" name=\"wea_mode\" %s>极简主题<input class=\"radio\" input type=\"radio\" value=\"1\" name=\"wea_mode\" %s>太空人主题</label>" \
                    "<label class=\"input\"><span>屏幕亮度(1~100)</span><input type=\"range\"name=\"backLight\"value=\"%s\"min=\"0\" max=\"100\"></label>" \
                    "<label class=\"input\"><span>氛围灯最低亮度</span><input type=\"range\"name=\"min_brightness\"value=\"%s\"min=\"0\" max=\"0.99\" step=\"0.01\"></label>" \
                    "<label class=\"input\"><span>氛围灯最高亮度</span><input type=\"range\"name=\"max_brightness\"value=\"%s\"min=\"0\" max=\"0.99\" step=\"0.01\"></label>" \
                    "<label class=\"input\"><span>氛围灯渐变时间(毫秒)</span><input type=\"text\"name=\"time\"value=\"%s\"></label>" \
                    "<label class=\"input\"><span>相册自动切换</span><input class=\"radio\" type=\"radio\" value=\"0\" name=\"switch\" %s>关闭<input class=\"radio\" input type=\"radio\" value=\"1\" name=\"switch\" %s>开启</label>" \
                    "<label class=\"input\"><span>相册切换时间(毫秒)</span><input type=\"text\"name=\"switchInterval\"value=\"%s\"></label>" \
                    "<label class=\"input\"><span>媒体播放器自动切换</span><input class=\"radio\" type=\"radio\" value=\"0\" name=\"switchFlag\" %s>关闭<input class=\"radio\" input type=\"radio\" value=\"1\" name=\"switchFlag\" %s>开启</label>" \
                    "<label class=\"input\"><span>高性能模式</span><input class=\"radio\" type=\"radio\" value=\"0\" name=\"power_mode\" %s>关闭<input class=\"radio\" input type=\"radio\" value=\"1\" name=\"power_mode\" %s>开启</label>" \
...
#define RGB_SETTING "<form method=\"GET\" action=\"saveRgbConf\">"                                                                                             \
                    ...
                    "<label class=\"input\"><span>RGB渐变时间（毫秒）</span><input type=\"text\"name=\"time\"value=\"%s\"></label>"           \
...
void init_page_header()
{
    ...
    webpage_header += F("body{max-width:65%;margin:0 auto;font-family:arial;font-size:100%;text-align:center;color:white;background-color:#014946}");
    webpage_header += F("ul{list-style-type:none;margin:0.1em;padding:0;overflow:hidden;background-color:#8F3430;font-size:1em;position:-webkit-sticky;position:sticky;top:0;}");
    webpage_header += F("li{float:left;border-right:0.06em solid #bbb;}last-child {border-right:none;}");
    webpage_header += F("li a{display: block;padding:14px 16px;text-decoration:none;color:white;}");
    webpage_header += F("li a:hover{background-color:white;}");
    webpage_header += F("section {font-size:0.88em;}");
    webpage_header += F("h1{color:white;border-radius:0.5em;font-size:1.2em;padding:0.2em 0.2em;background:#b1b1b1;}");
    ...
    webpage_header += F("footer{background-color:#b1b1b1; text-align:center;padding:0.3em 0.3em;border-radius:0.375em;font-size:60%;margin-top: 10px;}");
    webpage_header += F("button{border-radius:0.5em;background:#558ED5;padding:0.3em 0.3em;width:20%;color:white;font-size:100%;}");
    ...
    webpage_header += F("a{font-size:90%;}");
    webpage_header += F("p{font-size:75%;}");

    webpage_header += F("</style></head><body>"); // CSS样式定义结束，开始HTML主体部分

    webpage_header += F("<h1>My HoloCubic ");
    ...
    webpage_header += F("<li><a href="">未来功能 敬请期待</a></li>");
    webpage_header += F("<li style=\"float:right\"><a href='/About'>About</a></li>");
    webpage_header += F("</ul>");
}

void init_page_footer()
{
    webpage_footer = F("<footer>&copy;ClimbSnail 2021 Hynetek 2023</footer>");
...
void HomePage()
{
    ...
    webpage = F("<a href='https://www.bilibili.com/opus/880387090763743269?spm_id_from=333.999.0.0' target='_blank'><button>教程</button></a>");
    Send_HTML(webpage);
}
...
{
    webpage = "<p>About</p>";
    webpage += "<p>This is the About page. It provides information about the project.</p>";
    webpage += "<h2>小电视基于Holocubic，原为Up主稚晖君大佬的开源项目。</h2>";
    webpage += "<h2>同时小电视也参考了很多优秀的开源代码和设计思路，以下是一些参考来源。</h2>";
    webpage += "<h2>原作者的项目链接：<a href='https://github.com/peng-zhihui/HoloCubic' target='_blank'>https://github.com/peng-zhihui/HoloCubic</a></h2>";
    webpage += "<h2>参考项目的地址：<a href='https://github.com/ClimbSnail/HoloCubic_AIO' target='_blank'>https://github.com/ClimbSnail/HoloCubic_AIO</a></h2>";
    webpage += "<h2>感谢大家的帮助,如果涉及侵权请联系我们</h2>";
    webpage += "</body></html>";
    Send_HTML(webpage);
}
...
void sys_setting()
{
    ...
    sprintf(buf, SYS_SETTING, ssid_0, password_0, tianqi_addr,
            (cfg.auto_start_app == "Weather") ? "selected=\"selected\"" : "",
            (cfg.auto_start_app == "Picture") ? "selected=\"selected\"" : "",
            (cfg.auto_start_app == "Media") ? "selected=\"selected\"" : "",
            (strcmp(wea_mode, "0") == 0) ? "checked=\"checked\"" : "",
            (strcmp(wea_mode, "0") == 0) ? "" : "checked=\"checked\"",
            backLight, min_brightness, max_brightness, time,
            (strcmp(switchInterval, "0") == 0) ? "checked=\"checked\"" : "",
            (strcmp(switchInterval, "0") == 0) ? "" : "checked=\"checked\"",
            switchInterval,
            (strcmp(switchFlag, "0") == 0) ? "checked=\"checked\"" : "",
            (strcmp(switchFlag, "0") == 0) ? "" : "checked=\"checked\"",
            (strcmp(power_mode, "0") == 0) ? "checked=\"checked\"" : "",
            (strcmp(power_mode, "0") == 0) ? "" : "checked=\"checked\"");
    webpage = buf;
    Send_HTML(webpage);
}
...
void weather_setting()
{
    ...
    sprintf(buf, WEATHER_SETTING, tianqi_addr);
    webpage = buf;
    Send_HTML(webpage);
}
...
void anniversary_setting()
{
    ...
    webpage = buf;
    Send_HTML(webpage);
    Serial.println(F("  ----  anniversary setting success"));
}
...
void saveSysConf(void)
{
    ...
    if (strcmp(server.arg("auto_start_app").c_str(), "1") == 0)
    {
        app_controller->send_to(SERVER_APP_NAME, "AppCtrl",
                            APP_MESSAGE_SET_PARAM,
                            (void *)"auto_start_app",
    ...
    else if (strcmp(server.arg("auto_start_app").c_str(), "2") == 0)
    {
        app_controller->send_to(SERVER_APP_NAME, "AppCtrl",
                            APP_MESSAGE_SET_PARAM,
                            (void *)"auto_start_app",
    ...
    if (strcmp(server.arg("switch").c_str(), "0") == 0)
    {
    ...
    app_controller->send_to(SERVER_APP_NAME, "Weather",
                            APP_MESSAGE_SET_PARAM,
                            (void *)"wea_mode",
                            (void *)server.arg("wea_mode").c_str());
    // 持久化数据
